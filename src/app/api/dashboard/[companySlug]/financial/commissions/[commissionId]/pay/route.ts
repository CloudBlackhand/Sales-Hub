...
import { payCommission } from "@/server/actions/financial";
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const result = await payCommission(resolved.company.id, commissionId);
  if (!result.success) {
    return NextResponse.json({ error: result.error ?? "Erro ao pagar comissão" }, { status: 400 });
  }
